...
We trained a ResNet50 using the SupConLoss technique to classify plankton images. The DYB-PlanktonNet dataset was split into train, val, and test sets using `01_yang_split_dataset_linear_head.py`. The mean and std of the train set were computed with `02_mean_std_variables.py`. Backbone weights were selected by running `main_linear_loop.sh`. A FAISS + SQL database was then created for fast image queries using `04_lut_creation.py`, and the lookup table (LUT) was pruned with `05_lut_pruning.py`.

The script `07_run_threshold_experiments.py` automates the process of evaluating the impact of different similarity threshold values on the pruning of the lookup table (LUT) and subsequent inference performance. It iteratively updates the threshold parameter, runs the pruning and inference scripts, and collects key metrics such as accuracy, processing time, and FAISS index file size for each threshold value.

The script `08_thresholds_comparison.py` is used to visualize the results obtained from the threshold experiments. It loads the metrics collected for each threshold value and generates comparative plots, allowing for an analysis of how the similarity threshold affects classification accuracy, inference speed, and index size.
