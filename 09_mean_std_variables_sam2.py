...
def main():
    """
    Calculate mean and standard deviation of a dataset of images.
    
    This function loads images from a specified directory and calculates 
    the channel-wise mean and standard deviation using two different methods.
    
    Returns:
        tuple: Mean and standard deviation tensors from both calculation methods
    """
    # Assuming your images are stored in 'PATH_TO_IMAGES'
    PATH_TO_IMAGES = '/home/dsosatr/ofa/segment-anything/output/sam2_train_masks_yes'
    ...
    total_mean = mean2/count
    total_var = (meansq/count) - (total_mean**2)
    total_std = torch.sqrt(total_var)

    print(f'Method 2:\n Mean: {total_mean}, Std: {total_std}, Total pixels: {count}')
    
    return (mean, std, total_mean, total_std)

if __name__ == "__main__":
    """
    Compute the standard deviation and mean of an image dataset.
    ...
    This script uses two different methods to calculate means and standard deviations:
    - Method 1: Simple averaging of per-batch means and standard deviations
    - Method 2: More accurate calculation using sum of values and sum of squared values
    """
    main()
    ...
    # Mean and Std values for SAM2 selected and negative datasets.
    # Method 1:
    # Mean value: tensor([0.0425, 0.0436, 0.0432]), Std value: tensor([0.1379, 0.1411, 0.1406])
    # # Method 2:
    # Mean: tensor([0.0425, 0.0436, 0.0432]), Std: tensor([0.1466, 0.1488, 0.1476]), Total pixels: 1388119040   --> ESTE ES EL UTILIZADO