...
def parse_option():
    ...
    # optimization
    parser.add_argument('--learning_rate', type=float, default=0.016,
                        help='learning rate')
    ...
    parser.add_argument('--mean', type=str, default='0.0425, 0.0436, 0.0432', help='mean of dataset in path in form of str tuple')
    parser.add_argument('--std', type=str, default='0.1466, 0.1488, 0.1476', help='std of dataset in path in form of str tuple')
    parser.add_argument('--data_folder', type=str, default='/home/dsosatr/ofa/segment-anything/output/sam2_train_masks_yes', help='path to custom train dataset')
    parser.add_argument('--val_folder', type=str, default='/home/dsosatr/ofa/segment-anything/output/sam2_test_masks_yes', help='path to custom test dataset')
...
def set_model(opt):
    if torch.cuda.is_available():
        ...
        custom_model.load_state_dict(torch.load('pesos/resnet50_stage2.pth')['model'])

        classifier = LinearClassifier(name=opt.model, num_classes=87)
        classifier = classifier.cuda()
        #classifier.load_state_dict(torch.load(head_path)['model'])
        classifier.load_state_dict(torch.load('pesos/resnet50_stage2_head.pth')['model'])
